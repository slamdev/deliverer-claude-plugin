...
- [ ] `changeRequestUrl` is gone from the payload and its schema. The call that started the review names it in the same
      session record, and the **observer** takes a poll's review id off the call rather than off the answer, so nothing
      joins on this.
- [ ] `lastEventAt` is gone. `events` alone is the whole working-versus-wedged signal: the record's own update stamp
      moves only when an event is accepted and `events` increments, so two polls could never disagree about them.
- [ ] That update stamp itself stays, because eviction still reads it for a terminal record with no ending timestamp.
- [ ] `turns` and `canonicalModel` are gone from the payload, from the spend type, from its recorded form, from its
      no-spend value, from its merge, and from the real backend's extraction.
- [ ] The count of assistant messages that existed only as `turns`' fallback is gone, and so is the measurement comment
      that justified it. Nothing else counted turns.
- [ ] `deadlineSec` is gone, and no bound is published as a figure at all.
- [ ] The status tool's description names both bounds — the absolute cap, and the idle bound that ordinarily ends a
...
- [ ] The refusal a caller meets when a review is already in flight goes on naming both bounds, unchanged.
- [ ] Verified against the scripted backend: no answer carries any of the five keys, and the status tool's description
