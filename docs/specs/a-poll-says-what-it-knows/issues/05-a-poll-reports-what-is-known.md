...
- [ ] A key appears only when there is something to read: no `null`, no literal `unknown`, no empty string. This covers
...
- [ ] Four keys are always present because they are always known — the review's id, its status, when the record opened,
...
- [ ] `events` is published at zero. Nothing has landed is a measurement, not an absence.
- [ ] What a round spent is one object under the glossary's word for it, carrying the four token counters, the dollar
...
- [ ] The four token classes stay separate. They are the figure that does not depend on a provider's price list, and
      each of the four is priced differently.
- [ ] Time is not spend. How long the round ran inside the reviewer sits beside the record's own timestamps and the
      event count, outside that object.
- [ ] That duration stays the reviewer's own figure rather than the difference between two timestamps. Deriving it
...
- [ ] The observer reads the new shape and only the new shape. A debrief's round line says exactly what it says today —
...
- [ ] Replaying a record written before this ticket reports its rounds' spend as unknown. That is the accepted
      consequence, not a defect to chase, and nothing is added to read the old key.
- [ ] `code-reviewer` reports a round's total tokens and the provider-labelled dollar estimate, unknown where a key is
      absent, and stops carrying four token classes through prose into a human's report.
- [ ] The delivery skill's report line follows: each round's total tokens and its provider-labelled dollar estimate,
      unknown where a round has none.
- [ ] The status tool's description says that a key is present only when there is something to read.
- [ ] The example payload in the **harness**'s README matches what the server now sends, so the price-table oracle
      beside it still describes a real answer.
- [ ] Verified twice: the whole lifecycle against the scripted backend — running, completed, failed and cancelled —
