...
- [x] The build happy path asserts that every **round** which reached `completed` or `failed` has a poll answer
...
- [x] The keys it asserts on are written down in the **harness** rather than imported from the server, for the reason
...
- [x] Nothing else in the harness moves. The **ceiling**s, the **verifier**'s questions and every existing matcher are
      untouched.
- [x] The build happy path is run once, end to end, against a real forge, and passes with the new assertion in place.
- [x] What that run took and what it cost are reported. A **ceiling** reached is reported as a finding about the run
      rather than as a failing assertion, exactly as the harness already treats one.
- [x] `typecheck` and `lint` pass in both packages; the file's register and column width are matched.
