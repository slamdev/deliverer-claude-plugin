...
> *This was generated by AI while answering the review of PR #10.*

**The rule this ticket's `Progress` criterion rests on has since been counted per stage, and the criterion stays as the
record of what this ticket built.** It reads `One dispatch, one task`, which is what the skill said then. That wording
contradicted stage 1 — one task covers one `implementer` dispatch per ticket — so `plugin/skills/build/SKILL.md` now
reads `One stage, one task`, with a cold `code-reviewer` opening a fresh **round** as its one stated exception. What
this ticket asked for still holds: the sentence naming the paired stage's two tasks is gone, the worked example still
names a task from the stage it serves, and stage 1's accommodation is untouched. No criterion is reworded.

The criterion above saying earlier specs keep the stage numbers they were written with holds too, and this is not an
exception to it: `../spec.md` carries a dated note recording that the rule's name moved rather than pretending the old
wording was never shipped.
