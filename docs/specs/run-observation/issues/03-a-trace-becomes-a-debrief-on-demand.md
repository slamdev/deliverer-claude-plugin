...
- [ ] The run's extent is settled before a single figure is computed, and attribution does not settle it: it
...
- [ ] The header carries: which skill ran, the epic's slug, wall-clock, how many dispatches, how many **rounds** and
      ...
      plugin's commit.
- [ ] Wall-clock is the run's, and neither of the two figures lying nearest to hand. Taken from the entries carrying
...
- [ ] A dispatch count counts dispatches and not the files they leave. Thirteen dispatches leave 26 files — a record
      and a `.meta.json` sidecar each — and `../spec.md`'s Further Notes count the files.
- [ ] Rounds are counted per review id across the main record and every per-dispatch record, since the run's own
...
- [ ] How the run itself ended is stated: it finished, or it stopped, naming the stage it stopped in. The run most
      ...
      run ended.
- [ ] The run's own spend is tokens, per API request. The host records no money at all — no cost, dollar or price
...
- [ ] A figure nobody measured reads `unknown` and never zero. A figure measured at zero says zero: with no judging,
...
- [ ] What the observation lost is in the debrief. A trace that says what was truncated, malformed or unreadable
      carries that forward, so a run whose records were damaged does not read as a run with nothing wrong with it.
- [ ] A facts section carries the human's own time: how many question rounds the run put to them, and how long the
...
- [ ] The footer names where to send the debrief: an issue on the plugin's own repository, as one fixed destination
...
- [ ] The commit is the run's own where the records carry it, labelled as the commit the run used. The skill preamble
      ...
      exercised.
- [ ] Nothing runs `git` to obtain it. The installed plugin is not a checkout — its directory is named by the commit
      ...
      debrief.
- [ ] Replaying the same records twice produces the same debrief, byte for byte. Nothing in it records when the
...
- [ ] A replay of a run that already has a debrief writes beside it. Nothing already there is rewritten, appended to
      or removed, which is D19 holding for the debrief as it does for the trace.
- [ ] The debrief lands under the plugin's data directory beside the trace, nothing is written inside any repository,
      and `CLAUDE_PLUGIN_DATA` is required the way ticket 02's entry point requires it.
- [ ] An **identity file** lands beside each debrief, carrying the run's key — ticket 02's slug and first timestamp —
...
- [ ] That file refuses forwarding in its own filename and its own first line, the two places the trace already does —
...
- [ ] What lands is covered by the typecheck and lint ticket 02 got reaching the observer's code. No test runner is
...
- [ ] Verified against the records of every deliverer run on the machine this epic was written on — four when this
