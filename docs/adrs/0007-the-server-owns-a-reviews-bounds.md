...
One review runs at a time, and every review is bounded twice by figures the server states: an idle timeout of fifteen
...
the last event — fifteen minutes is four times the largest average gap ever observed between two of them. The absolute
cap catches nothing on its own account; it exists so that "never" stays out of reach, because an inner agent emitting
one event a minute for a week resets the idle clock every time. Four hours is above any round anyone has seen finish and
still finite.
...
did — so the bound that catches the failure now measures exactly that: fifteen minutes with nothing arriving. The hour
