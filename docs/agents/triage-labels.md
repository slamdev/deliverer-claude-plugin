...
This repo tracks issues as local markdown (see `issue-tracker.md`), so a "label" is not applied through a tracker API —
it is written as the `Status:` line near the top of the issue file, e.g. `Status: ready-for-agent`.
