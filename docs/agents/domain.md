...
This repo is **single-context**: one `CONTEXT.md` and one `docs/adrs/` at the root.
...
If this repo ever grows into several bounded contexts, the layout becomes a root `CONTEXT-MAP.md` pointing at one
`CONTEXT.md` per context, with context-scoped ADRs alongside each. Until that file exists, treat the repo as
single-context.
