...
#
# It reads its options from NOTHING. `CLAUDE_PLUGIN_OPTION_<KEY>` reaches a hook only when the option
# was explicitly set — at the manifest default the variable is absent entirely (measured, ticket 02) —
# so a hook must not read one to learn an effective value. Effort, model and the review's environment
# file reach the server through the MCP `env` map instead, which has no such gap.
set -uo pipefail
...
# Checked unconditionally: a plugin update can change the server's source without touching its
# dependency manifest, and the copy is a handful of small files.
#
# `$DATA/server` is a SYMLINK to a per-process directory, published with `ln -sfn`, which replaces
...
#
# Consumers, both checked: `mcp/launch.mjs` resolves `$DATA/server/index.ts`, and Node resolves ESM
# imports straight through a symlinked directory (the modules it needs sit beside it in
# `$DATA/node_modules`, reached from the REAL path, which is a sibling — so nothing depends on the
# link being preserved in the resolved path). And the leftover sweep below knows all three names, so
# neither the link nor the tree it points at is ever swept as debris.
if [[ -d "$SRC/server" ]]; then
  # The staging and published paths are PER PROCESS. Concurrent SessionStart hooks in one data
  # directory are routine (two windows on one host, or a suite driving two sessions), and this block
  # runs unconditionally in every one of them: with fixed names, the second hook's opening `rm -rf`
  ...
  STAGED="$DATA/server.staged.$$"
  PUBLISHED="$DATA/server.d.$$"
  LINK="$DATA/server"
  rm -rf "$STAGED"
  ...
  # long dead the pid that published it, because that one is the running server's source.
  current=""
  [[ -L "$LINK" ]] && current="$(readlink "$LINK" 2>/dev/null || true)"
  for leftover in "$DATA"/server.staged.* "$DATA"/server.d.*; do
    [[ -e "$leftover" ]] || continue
  ...
  if cp -R "$SRC/server" "$STAGED"; then
    if [[ -L "$LINK" ]] && diff -r -q "$STAGED" "$LINK" >/dev/null 2>&1; then
      ...
      if [[ "$PUBLISHED" == "$current" ]]; then
        publish_seq=2
        while [[ -e "$DATA/server.d.$publish_seq.$$" ]]; do publish_seq=$(( publish_seq + 1 )); done
        PUBLISHED="$DATA/server.d.$publish_seq.$$"
      fi
      rm -rf "$PUBLISHED"
      mv "$STAGED" "$PUBLISHED"
    ...
    rm -rf "$STAGED"
    warn "cannot copy the tools server's source into $DATA — the server will not start."
  fi
else
