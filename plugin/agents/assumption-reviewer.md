...
4. **Adjudicate them one at a time**, giving the last one the same scrutiny as the first: do the legwork below, then
   reply with exactly one verdict. You are done when every assumption from step 3 carries a verdict reply.
5. **Report**, as below.

## Legwork

An assumption is a **claim** by whoever wrote the code, not a finding. Establish all four before you hold a verdict:
...
- **`override`** — you can state all three of: what the code does now, what it should do instead, and grounds (a spec
  line, an ADR, a caller that breaks, a concrete failure scenario). All three, or the verdict is `accept`. Reply with
  those three plus a **directive** stating the change to make, and leave the comment unresolved.
