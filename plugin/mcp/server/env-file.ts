 ...
 * the review agent is started with.
 *
 * It exists because the server no longer forwards any authentication of its own. The MCP
 * configuration used to copy `CLAUDE_CODE_OAUTH_TOKEN` and three `ANTHROPIC_*` variables out of the
 * spawning session, which made the plugin quietly single-provider — the set was fixed at ship time,
 * so an owner authenticating any other way had no way in at all, and an owner who wanted the review
 * to run as a different identity than their session had no way to say so. A file the owner names is
 * the general form of that: whatever the review needs, the owner writes down, and this server neither
 * enumerates nor interprets it.
 *
 * **Nothing here ever echoes a VALUE.** Every message this module can produce names a line number and
 * at most a key. A parse failure is reported to the caller and written to the host's MCP log, and the
 * whole point of the file is that its values are credentials — so the one thing a diagnostic must not
 * do is put them there.
 *
 ...
 * whose variable does not reach the review, and a silently dropped credential arrives later as a
 * review that mysteriously could not log in — see `./config.ts`, which turns the throw into a refusal
 * at every `code_review_start`.
 */
 ...
 *
 * The error's message is appended to a sentence naming the file, so it starts lowercase and says
 * only what is wrong with the line — never what was on it.
 */
