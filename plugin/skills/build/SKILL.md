...
rather than going to look — the change request's comments and its checks are the dispatched agent's to read, and the
bearings you take from the branch and the change request say which stage is owed, not what a finding says or whether a
check is green. Yours is the work no agent does: read the epic for its tickets, dispatch in order, read each report,
keep the task list current, and flip the change request ready at the end.

**Resume.** This epic may be part-delivered — by an earlier run of your own that was interrupted, or by hand. The epic
branch and the change request are what say how far it got: the commits name the tickets already implemented; whether a
change request is open for the branch says whether stage 2 ran; an `ASSUMPTION` comment carrying a **verdict** reply says
stage 3 adjudicated it; unresolved comments say a **fix wave** is still owed. Start from the earliest stage whose
evidence you cannot find — re-dispatching any agent but `code-reviewer` is safe, because each one resumes on its own and
adds only what is missing. `code-reviewer` is the exception: see **Rounds**.
...
1. **Implement every ticket** — dispatch `implementer` once per ticket, in the order the epic lists them, one ticket at
   ...
   the epic and that one ticket. You are done when every ticket the epic lists has a report naming its commits.
2. **Open the change request** — dispatch `change-request-creator` with the epic. It opens the change request as a
   **draft** and mirrors every assumption the branch's commits recorded into an `ASSUMPTION` comment. Keep the URL it
   reports and pass it to every dispatch below, so none of them has to find it again.
3. **Adjudicate and review together** — dispatch `assumption-reviewer` and `code-reviewer` in one message so they run
   concurrently, each with the epic and the URL. `assumption-reviewer` replies a verdict — `accept`, `override` or
   `escalate` — to every `ASSUMPTION` comment. `code-reviewer` drives one **round**, whose findings the tools server
   posts as comments itself.
4. **First fix wave** — dispatch `comments-addresser` with the epic and the URL. It works every unresolved comment:
   implementing the overrides and the findings, resolving what it implements or declines, and leaving the escalations for
   a human. Keep the **hand-off** list its report carries.
5. **Second round** — dispatch `code-reviewer` again, with the epic and the URL. It reviews the fix wave's commits, so
   what the first round missed still gets caught.
...
the tickets the commits account for, so a resumed run opens at the count it left off.
